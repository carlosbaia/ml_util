...
def cross_val_predict_proba(classifier, X, y, cv=3, random_state=None, verbose=0):
    ...
    predict_proba = _np.empty(y.shape)
    for i, (index_train, index_test) in enumerate(_StratifiedKFold(y, n_folds=cv, random_state=random_state), 1):
        start_time = _dt.now()
        if verbose > 0:
        ...
        try:
            predict_proba[index_test] = classifier.predict_proba(X[index_test])[:, 1]
